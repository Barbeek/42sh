...
/*   Updated: 2019/03/11 18:02:48 by aroblin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void				apply_rule(t_varsexp *vexp)
{
	...
	*pos = 0;
	...
	(val = get_val(vexp->data->spe, vexp->str))
	|| (val = get_val(vexp->data->loc, vexp->str))
	|| (val = get_val(vexp->data->env, vexp->str));
	if (!val)
...
static	char			*exp_res_rules(t_varsexp *vexp)
{
	...
	{
		(tmp = get_val(vexp->data->spe, vexp->str))
		|| (tmp = get_val(vexp->data->loc, vexp->str))
		|| (tmp = get_val(vexp->data->env, vexp->str));
